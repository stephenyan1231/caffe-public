...
void CompactProbabilisticAverageProbLayer<Dtype>::Reshape(
	...
	(top)[0]->Reshape(bottom[prob_num_]->num(), class_num_, 1, 1);
...
void CompactProbabilisticAverageProbLayer<Dtype>::Backward_cpu(
	...
	if (propagate_down[0]) {
		memset(branch_prob_diff, 0,
				sizeof(Dtype) * (bottom)[prob_num_]->count());
		for (int i = 0; i < num; ++i) {
			for (int j = 0; j < prob_num_; ++j) {
				...
				Dtype prob_weight = (branch_prob_data
						+ (bottom)[prob_num_]->offset(i))[j];
				for (int k = 0; k < compact_layer_size_[j]; ++k) {
					...
					(branch_prob_diff + (bottom)[prob_num_]->offset(i))[j] +=
							top_diff_i[class_id]
									* (bottom_data + (bottom)[j]->offset(i))[k];
				}
			}
		}
	}
