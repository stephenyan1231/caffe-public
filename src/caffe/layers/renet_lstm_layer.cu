...
__device__ Dtype sigmoid_dev(Dtype x);
...
template<>
__device__ double sigmoid_dev(double x) {
  return 1. / (1. + exp(-x));
}

template<typename Dtype>
__device__ Dtype sigmoid_diff_y_dev(Dtype y) {
  return y * (1.0 - y);
}

template<typename Dtype>
__device__ Dtype tanh_dev(Dtype x) {
  return 2. * sigmoid_dev<Dtype>(2. * x) - 1.;
}

template<typename Dtype>
__device__ Dtype tanh_diff_x_dev(Dtype x) {
  Dtype y = tanh_dev<Dtype>(x);
  return 1.0 - y * y;
}

template<typename Dtype>
__device__ Dtype tanh_diff_y_dev(Dtype y) {
  return 1.0 - y * y;
}

__device__ int blob_offset(int channels, int height, int width, int n, int ch,
    int y, int x) {
...
template<typename Dtype>
__global__ void compute_cell_state(ReNetLSTMParameter::Direction dir,
...
void ReNetLSTMLayer<Dtype>::ComputeCellData_gpu(int dir_num, int step_id,
  ...
  caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_RNN_ * num_,
      ...
      this->blobs_[dir_num * num_blobs_per_dir_ + 4]->gpu_data(), (Dtype) 1.,
      gi_data);
  ...
  caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_RNN_ * num_,
      num_output_, 1, (Dtype) 1., bias_multiplier_.gpu_data(),
      this->blobs_[dir_num * num_blobs_per_dir_ + 5]->gpu_data(), (Dtype) 1.,
      ci_data);
  ...
  caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_RNN_ * num_,
      num_output_, 1, (Dtype) 1., bias_multiplier_.gpu_data(),
      this->blobs_[dir_num * num_blobs_per_dir_ + 6]->gpu_data(), (Dtype) 1.,
      go_data);
  ...
  compute_cell_state<Dtype> <<<CAFFE_GET_BLOCKS(num_RNN_ * num_ * num_output_),
...
template<typename Dtype>
__global__ void ComputeCellDiff(ReNetLSTMParameter::Direction dir, int dir_num,
    int patch_ny, int patch_nx, int num, int patch_h, int patch_w,
...
void ReNetLSTMLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int dir_num = 0; dir_num < 2; ++dir_num) {
    ...
    for (int step_id = step_end; step_id >= step_min && step_id <= step_max;
      ...
      ComputeCellDiff<Dtype> <<<
          CAFFE_GET_BLOCKS(num_RNN_ * num_ * num_output_),
          CAFFE_CUDA_NUM_THREADS>>>(dir_, dir_num, patch_ny_, patch_nx_, num_,
