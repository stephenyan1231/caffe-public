...
void BaseConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	quantization_kmean_num_cluster_ =
			this->layer_param_.convolution_param().quantization_kmean_num_cluster();
	quantization_num_segment_ =
			this->layer_param_.convolution_param().quantization_num_segment();
	quantization_kmean_cluster_centers_file_ =
			this->layer_param_.convolution_param().quantization_kmean_cluster_centers_file();
	quantization_kmean_cluster_indices_file_ =
			this->layer_param_.convolution_param().quantization_kmean_cluster_indices_file();
	if (this->parameter_compress_ && quantization_kmean_num_cluster_ > 0) {
		...
		LOG(INFO) << "Layer " << this->layer_param_.name()
				<< " read quantization_kmean_cluster_centers_ from "
				<< quantization_kmean_cluster_centers_file_;
		BlobProto centers_blob_proto;
		ReadProtoFromBinaryFile(quantization_kmean_cluster_centers_file_,
				&centers_blob_proto);
		quantization_kmean_cluster_centers_.FromProto(centers_blob_proto);
		LOG(INFO) << "quantization_kmean_cluster_centers_ height "
				<< quantization_kmean_cluster_centers_.height() << " width "
				<< quantization_kmean_cluster_centers_.width();

		BlobProto indices_blob_proto;
		ReadProtoFromBinaryFile(quantization_kmean_cluster_indices_file_,
				&indices_blob_proto);
		quantization_kmean_cluster_indices_.FromProto(indices_blob_proto);
		LOG(INFO) << "quantization_kmean_cluster_indices_ height "
				<< quantization_kmean_cluster_indices_.height() << " width "
...
void BaseConvolutionLayer<Dtype>::weight_cpu_gemm(const Dtype* input,
	...
	for (int g = 0; g < group_; ++g) {
		caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, conv_out_channels_ / group_,
				...
				output + output_offset_ * g, col_buff + col_offset_ * g, (Dtype) 1.,
