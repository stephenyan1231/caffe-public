...
void ReadLabelFile(const string& label_file_name, int img_height, int img_width,
...
bool ReadImageToSemanticLabelingDatum(const string& img_name,
		const string& label_file_name, const int min_height, const int min_width,
		SemanticLabelingDatum* datum) {
	DLOG(INFO)<<"read image: "<<img_name;
	cv::Mat cv_original_img = ReadImageToCVMat(img_name, 0, 0, true);
	DLOG(INFO)<<"reading succeeds";
	...
	DLOG(INFO)<<"ReadImageToSemanticLabelingDatum resize image from ("
	...
	vector<uchar> buf;
	DLOG(INFO)<<"encode image into JPEG format";
	cv::imencode(".jpg", cv_img, buf);
	DLOG(INFO)<<"encoding succeeds";
	datum->Clear();
...
cv::Mat DecodeSemanticLabelingDatumToCVMatNative(
		const SemanticLabelingDatum& datum) {
	cv::Mat cv_img;
	CHECK(datum.encoded()) << "Datum not encoded";
	const string& data = datum.data();
	std::vector<char> vec_data(data.c_str(), data.c_str() + data.size());
