...
cv::Mat ReadImageToCVMatShortLongSide(const string& filename, const int short_side_size,
		const int long_side_size, const bool is_color) {
	cv::Mat cv_img;
	int cv_read_flag = (is_color ? CV_LOAD_IMAGE_COLOR : CV_LOAD_IMAGE_GRAYSCALE);
	cv::Mat cv_img_origin = cv::imread(filename, cv_read_flag);
	if (!cv_img_origin.data) {
	...
	if (short_side_size > 0) {
		if (cv_img_origin.rows < cv_img_origin.cols) {
			...
			height = ceil(
					(float(cv_img_origin.rows) / float(cv_img_origin.cols)) * width);
		}
		cv::resize(cv_img_origin, cv_img, cv::Size(width, height));
	} else if (long_side_size > 0) {
		if (cv_img_origin.rows < cv_img_origin.cols) {
...
bool ReadImageToDatumShortLongSide(const string& filename, const int label,
		const int short_side_size, const int long_side_size, const bool is_color,
		Datum* datum) {
	cv::Mat cv_img = ReadImageToCVMatShortLongSide(filename, short_side_size, long_side_size,
			is_color);
	if (cv_img.data) {
		CVMatToDatum(cv_img, datum);
		datum->set_label(label);
...
cv::Mat DecodeDatumToCVMat(const Datum& datum, const int height,
		const int width, const bool is_color) {
	cv::Mat cv_img;
...
cv::Mat DecodeDatumToCVMat(const Datum& datum, const bool is_color) {
