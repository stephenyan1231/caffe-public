...
void convert_dataset(const string& input_folder, const string& output_folder,
  ...
  if (labels_of_interest == "") {
    ...
    std::ifstream labels_file((input_folder + "/" + labels_of_interest).c_str(),
        std::ios::in);
    string line;
  ...
  train_db->Open((output_folder + string("/cifar100-train-") + backend).c_str(),
      db::NEW);
  scoped_ptr<db::Transaction> train_txn(train_db->NewTransaction(false));

  // Open files
  std::ofstream tr_img_list((input_folder + "/" + train_list_file).c_str(),
      std::ios::out);
  CHECK(tr_img_list) << "Unable to open " << train_list_file << " to write";
  int itemid = 0;
  for (int i = 0; i < kCIFARBatchNum; ++i) {
    snprintf(str_buffer, kCIFARImageNBytes, "/float_data_batch_%d.bin", i + 1);
    ...
    for (int j = 0; j < kCIFARBatchSize; ++j, ++itemid) {
      ...
      if (!interesting_labels
          || (interesting_labels && label_inte_bit[fine_label])) {
  ...
  test_db->Open((output_folder + string("/cifar100-test-") + backend).c_str(),
      db::NEW);
  scoped_ptr<db::Transaction> test_txn(test_db->NewTransaction(false));
  // Open files
  std::ifstream ts_data_file((input_folder + "/float_test_batch.bin").c_str(),
      std::ios::in | std::ios::binary);
  std::ofstream ts_img_list((input_folder + "/" + test_list_file).c_str(),
      std::ios::out);
