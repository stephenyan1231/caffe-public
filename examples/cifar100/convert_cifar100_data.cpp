...
void read_image(std::ifstream* file, int* coarse_label, int* fine_label,
		char* buffer) {
	char label_char;
...
void convert_dataset(const string& input_folder, const string& output_folder,
	...
	for (int itemid = 0; itemid < kCIFARTrainSize; ++itemid) {
		read_image(&tr_data_file, &coarse_label, &fine_label, str_buffer);
	...
	// Open files
	std::ifstream ts_data_file((input_folder + "/test.bin").c_str(), std::ios::in | std::ios::binary);
	...
	for (int itemid = 0; itemid < kCIFARTestSize; ++itemid) {
		read_image(&ts_data_file, &coarse_label, &fine_label, str_buffer);
		if(!interesting_labels || (interesting_labels && label_2_label.find(fine_label) != label_2_label.end())){
			int new_label = fine_label;
			if(interesting_labels && label_2_label_flag)
				new_label = label_2_label[fine_label];
...
int main(int argc, char** argv) {
	if (argc != 5 && argc != 6 && argc != 7) {
		printf("This script converts the CIFAR 100 dataset to the leveldb format used\n"
				...
				"Usage:\n"
				"    convert_cifar_data input_folder output_folder train_list_file test_list_file labels_of_interest[optional] label_2_label_flag[optional]\n"
