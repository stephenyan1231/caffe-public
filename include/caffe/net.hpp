...
class Net {
public:
	explicit Net(const NetParameter& param, Solver<Dtype> *solver = NULL,
			SolverParameter solver_param = SolverParameter());
	explicit Net(const string& param_file, Solver<Dtype> *solver = NULL,
			SolverParameter solver_param = SolverParameter());
	virtual ~Net();
	...
	void CopyTrainedLayersFromSuffixMatch(
	...
	inline const vector<string>& layer_names() const {
		CHECK_GT(net_threads_.size(), 0);
		return net_threads_[0]->layer_names();
	}
	...
	void clear_blobs_gpu();

	BaseDataManager<Dtype>* GetDataManager() {
	...
	SolverParameter& get_solver_param() {
		return solver_param_;
	}

	Solver<Dtype> *get_solver() {
		return solver_;
	}
...
protected:
	void ForwardBackwardHelper(const vector<Blob<Dtype>*>& bottom, Dtype *loss,
			bool do_backward);
...
public:
	...
	inline const vector<int>& params_shard_size() {
		return params_shard_size_;
	}
	...
	void set_work_message(shared_ptr<WorkMessage> work_message) {
		work_message_ = work_message;
	}

	Solver<Dtype> * GetExternalSolver() {
		return net_->get_solver();
	}
...
//  const vector<Blob<Dtype>* > *bottom_;
	...
	Dtype loss_;
	Net<Dtype>* net_;
	shared_ptr<WorkMessage> work_message_;
